...
# surv.t : vector of variables 
# cens : vector if indicators of censoring (0=censored, 1 = uncensored)
# f : survival model formula
# data : df containing variables in the model
...
# maxCol: a number specifying the maximum number of columns in design matrix (including
#   intercept) to be kept.
# prior.param: a vector of prior probabilities that parameters are non-zero. Default puts a prior
...
#   OR.fix is 2.
# nbest: a value specifying the number of models of each size returned to bic.glm by the
#   modified leaps algorithm.
# factor.type: a logical value specifying how variables of class "factor" are handled. A factor
#   variable with d levels is turned into (d-1) dummy variables using a treatment
#   contrast. If factor.type = TRUE, models will contain either all or none of
    ...
    factor.type = TRUE, factor.prior.adjust = FALSE, call = NULL, ...) 
{
    ...
    # how it moves through model space: defined in Fortran
        # info : numpy array
        # coef: numpy array
        # names.arg
        # nbest: 150 default
        ...
        # get all rows but only columns up to ncols
        rt[, 1:n.cols] <- rr
